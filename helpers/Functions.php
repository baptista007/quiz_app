...
function render_error($data = null, $code = 500) {
    header("HTTP/1.1 $code $data", false, $code);
    exit;
}
...
//            $statustext = "You are not authorized to access this page. Please contact system administrator for more information";
...
//    }
    ...
    if (user_login_status() == true) {
        ...
        //authenticate user
        if (is_post_request()) {
            $post_data = transform_request_data($_POST);
            $model = new BaseModel;
            $user = $model->get_user(array("username" => $post_data['username']));

            if (!empty($user)) {
                if (password_verify($post_data['password'], $user['password_hash'])) {
                    set_session("user_data", $user);
                    $redirect_to = (!empty($_SESSION['login_redirect_url']) ? $_SESSION['login_redirect_url'] : "main/index");
                    redirect_to_page($redirect_to);
                    exit;
                } else {
                    $view->page_error = "Invalid username or password";
                }
            } else {
                $view->page_error = "User not found";
            }
        } else {
        ...
        $view->render("main/login.php", null, "login_layout.php");
        exit;
    }
}
...
function ajaxFormPostOutcome($errors, $redirect_url = null, $field_messages = null, $success_message = null, $vars = null) {
    if (!empty($errors)) {
        if (is_array($errors)) {
            $msg = "<ul>";
        ...
        $msg = (!empty($success_message) ? $success_message : 'Data saved successfully.');
    }
    ...
    echo json_encode(
            array(
                ...
                'message' => $msg,
                'redirectUrl' => (!empty($redirect_url) ? $redirect_url : ''),
                'fieldmessages' => (!empty($field_messages) ? $field_messages : '')
            )
    );
}

function getUploadStatusDesc($status) {
    switch ($status) {
        case DataUploadStatus::draft:
            return 'Draft';
        case DataUploadStatus::reviewed:
            return 'Reviewed';
        case DataUploadStatus::rejected:
            return 'Rejected';
        case DataUploadStatus::approved:
            return 'Approved';
        case DataUploadStatus::published:
            return 'Published';
        case DataUploadStatus::submitted:
